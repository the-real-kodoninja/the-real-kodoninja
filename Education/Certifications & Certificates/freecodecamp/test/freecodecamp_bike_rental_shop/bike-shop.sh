...
RENT_MENU() {
  ...
  then
    ...
    then
      ...
      BIKE_AVAILABILITY=$($PSQL "SELECT available FROM bikes WHERE bike_id = $BIKE_ID_TO_RENT AND available = 't'")
      # if not available
      if [[ -z $BIKE_AVAILABILITY ]]
        ...
        MAIN_MENU "That bike is not available."
          ...
          SET_TO_FALSE_RESULT=$($PSQL "UPDATE bikes SET available = 'f' WHERE bike_id = $BIKE_ID_TO_RENT")
          # get bike info
        ...
        if [[ ! $BIKE_ID_TO_RETURN =~ ^[0-9]+$ ]]
          ...
          if [[ -z $RENTAL_ID ]]
            then
              ...
              SET_TO_TRUE_RESULT=$($PSQL "UPDATE bikes SET available = 't' WHERE bike_id = $BIKE_ID_TO_RETURN")
              #send to main menu
